...
**Fase 8 — validado ✅:** clamp do arroz ficou SENSÍVEL À REFEIÇÃO: almoço `[110,180]` / jantar `[60,110]` / default `[80,160]` (`stapleClamp(staple,meal)`). Resultado: arroz almoço **110g constante** (~5 colheres) e jantar ≤ almoço em TODOS os dias — inversão eliminada. Almoço ~440 kcal (principal), jantar ~250 (leve).
**Fase 8 — pendente:** refino #2 (alguns dias +8 a +16% da meta — ex. dia c/ omelete/peixe no jantar `owns:kcal` sem teto; e feijão às vezes cai a 30g pela correção). Refino #3 (`semente de abóbora` — usar `id` fixo no seed agora que `match-curated` aceita).

**Fase 9 (Biblioteca de Receitas — EM ANDAMENTO):** `backend/scripts/scrape-receiteria.js` (collect/ingest/stats). Lê JSON-LD schema.org `Recipe` do receiteria.com.br, decompõe ingredientes na TACO (20 micros completos), grava `recipes`/`recipe_ingredients`/`recipe_sources`. Só dados factuais (ingredientes/gramas/nutrição), nunca o preparo. Match endurecido: relaxamento progressivo de palavras, exclui marcas (`nome NOT LIKE '%(%'`), stopwords de descritores, des-penaliza creme de leite. **Portão de cross-check:** receita só fica `active` se cobertura ≥70% E meu kcal/100g bater ±20% com o do site; `healthy` se ainda passar no filtro fit. **Validado:** 754 URLs coletadas; ingest real de saladas → 13 confiáveis (kcal a ±10-15%, cobertura 77-100%): Coleslaw 43vs49, Purê abóbora 93vs97, Caprese, Ovo mollet, Baião de dois 221vs267. Taxa ~20% por categoria saudável; carnes rendem ~0 (não-fit + difíceis → portão reprova corretamente).
**Fase 9 — feito (ingest):** varredura ampla (13 categorias) → 726 processadas, **283 confiáveis, 45 fit** (após `reclassify` com filtro por ingredientes decompostos: exclui creme de leite/catupiry/maionese/açúcar/parmegiana/doces). Cobertura média 98%. LLM híbrido (Mistral) elevou aproveitamento de ~20% (regex) para ~33%. Fit por refeição: almoço/jantar 25, lanche 11, café 4, bebida 3, ceia 0.
**Fase 9c — feito (integração híbrida):** `fetchRecipePool` (receitas active+healthy decompostas → ingredientes c/ per100+micros da TACO). `generatePlan` alterna por dia: ~metade receita inteira escalada à kcal da refeição, ~metade montagem por archetype. Receitas SÓ em almoço/jantar/lanche (`RECIPE_USABLE`); café/ceia seguem archetype idiomático. Endpoint anexa `pool._recipes`. Validado: dia 1538-1598 vs alvo 1500 (±6%), café=Pão+queijo+café, almoço=Galinhada/Arroz+feijão+carne, lanche=Crepioca/Bolinho salgado. Receita não é mexida pela compensação de micros (itens sem `role`).
**Fase 9c — feito (preparo + nome do prato):** builder exibe `meal.archetype` como nome do prato (🍽️ dourado) no cabeçalho da refeição (`.wd-meal-dish`). Coluna `recipes.preparo` + comando `prep` gera MODO DE PREPARO PRÓPRIO via LLM (não copia o do site) a partir de nome+ingredientes decompostos; gerado p/ as 45 fit. `fetchRecipePool` traz `preparo`; receita injeta no `meal.instructions` → aparece em "Observações da refeição" no builder e no app/PDF do paciente. Validado.
**Fase 9 — feito (classificador de refeição):** `classifyMeal(name, ingredientes)` no `reclassify` reclassifica o `meal` por NOME + proteína (não pela categoria grosseira do site). Resultado: almoço/jantar só com pratos principais (Galinhada, Escondidinho, Bife de panela, Bobó, Risotos, Yakisoba…); "Salada de frutas no copo" → lanche; sopas → tag `jantar`. Distribuição fit: almoço/jantar 26, lanche 13, café/lanche 6.
**Fase 9 — refino pendente:** `healthy` ainda deixa passar uns nomes gordurosos (ex.: "Frango ao forno com maionese", "Salada caesar") — maionese/catupiry no NOME não bloqueiam (só nos ingredientes casados); add ao `BAD_NAME`. Reprocessar 11 pendentes + 10 falhas do ingest. Café/ceia poderiam ganhar receitas próprias se a biblioteca crescer.
